...
function setNavigation() {
	...
	$('#sidebar ul li ul li a').each(function () {
		...
		if ($this.attr('href') == current) {
			...
			$this.closest('.sidebar_menu').siblings('.dropdown-toggle').addClass('active');
			$this.addClass('active');
		}
	});
	...
	if(current == url+'/'+'exam-attendance-create'){
		...
		$('#subMenuExam').closest('.sidebar_menu').siblings('.dropdown-toggle').addClass('active');
