<?php
return [
"student_info" => "Info-étudiant",

"my_profile" => "Mon profil",

"subject_wise_attendance" => "Objet Sages Participation",

"subject_wise_attendance_report" => "Rapport sur la fréquentation des Sages",

"save_student_category" => "Enregistrer la catégorie d'étudiant",

"edit_student_category" => "Editer la catégorie d'étudiant",

"update_student_category" => "Mettre à jour catégorie d'étudiants",

"delete_student_category" => "Supprimer la catégorie d'étudiant",

"add_student_category" => "Ajouter une catégorie d'étudiant",

"age" => "Age",

"grade_point" => "Point de classe",

"without" => "Sans",

"point" => "Point",

"controller" => "Contrôleur",

"history" => "Historique",

"subject_attendance_report" => "Rapport sur la participation des sujets",

"ltl" => "LTL",

"rtl" => "RTL",

"alignment" => "Alignement",

"new_client_information" => "Nouveaux renseignements sur le client",

"made" => "Fait",

"final" => "Final",

"contribution" => "Contribution",

"transport_and_dormitory_details" => "Détails du transport et de la dortoir",

"driver_phone_number" => "Numéro de téléphone du pilote",

"national_identification_number" => "Numéro d'identification national",

"local_identification_number" => "Numéro d'identification local",

"UBC56987" => "UBC56987",

"class_period" => "Période de classe",

"paystack" => "Pile de paye",

"membership" => "Appartenance",

"sale_to" => "Vente à",

"available_for_online_registration" => "Disponible pour l'enregistrement en ligne",

"registration" => "Inscription",

"half_width" => "Demi-largeur",

"full_width" => "Largeur complète",

"one_thired_width" => "Largeur souhaitée",

"one_fourth_width" => "One Fourth Width",

"min_value" => "Valeur minimale",

"max_value" => "Valeur maximale",

"max_length" => "Longueur maximale",

"min_length" => "Longueur min",

"text_input" => "Entrée de texte",

"numeric_input" => "Entrée numérique",

"multiline_input" => "Entrée multiligne",

"datepicker_input" => "Entrée Datepicker",

"checkbox_input" => "Entrée de case à cocher",

"radio_input" => "Entrée radio",

"dropdown_input" => "Exploration en aval",

"file_input" => "Entrée de fichier",

"contarct_type" => "Type de contarct",

"student_information" => "Info-étudiant",

"add_custom_field" => "Ajouter une zone personnalisée",

"edit_custom_field" => "Editer une zone personnalisée",

"custom_field_list" => "Liste de zones personnalisées",

"admission_number" => "Numéro d'admission",

"admission_date" => "Date d'admission",

"add_time_setup" => "Ajouter une configuration de temps",

"edit_time_setup" => "Modification de l'heure d'installation",

"save_time_setup" => "Configuration de l'heure de sauvegarde",

"update_time_setup" => "Mise à jour de l'heure",

"delete_time_setup" => "Suppression de l'heure d'installation",

"time_setup_list" => "Liste de configuration du temps",

"transcript_no" => "Transcription Non",

"student_export" => "Exportation des étudiants",

"add_parents" => "Ajouter des parents",

"import" => "Importer",

"personal" => "Personnel",

"first" => "Première",

"last" => "Dernier",

"religion" => "Religion",

"caste" => "Goût",

"height_in" => "Hauteur (In)",

"weight_kg" => "Poids (KG)",

"weight" => "Poids",

"student_registration_custom_field" => "Zone personnalisée de l'inscription de l'étudiant",

"sibling" => "Sibling",

"&" => "&",

"occupation" => "Profession",

"Other" => "Autres",

"Other_info" => "Autres informations",

"other_info" => "Autres informations",

"other_information" => "Autres informations",

"current" => "Courant",

"route_list" => "Liste des routes",

"driver" => "Pilote",

"present_address" => "Adresse actuelle",

"national_id" => "Identité nationale",

"local_id" => "Identité locale",

"previous_school_details" => "Détails de l'école précédente",

"additional_notes" => "Remarques supplémentaires",

"parents_and_guardian_info" => "RENSEIGNEMENTS SUR LES PARENTS ET LES TUTEURS",

"transport_and_dormitory_info" => "Transport & Dortoir Info",

"document_01_title" => "Titre du document 01",

"document_02_title" => "Titre du document 02",

"document_03_title" => "Titre du document 03",

"document_04_title" => "Titre du document 04",

"student_details" => "Détails de l'élève",

"search_by_roll_no" => "Rechercher par annulation",

"fathers_name" => "Nom du père",

"mothers_name" => "Nom de la mère",

"father_phone" => "Père Téléphone",

"mother_phone" => "Mère Téléphone",

"select_sibling" => "Sélectionner un élément apparant",

"select_current_session" => "Sélectionner l'année académique en cours",

"promote_student_in_next_session" => "Promouvoir les étudiants au cours de la prochaine année universitaire",

"view_academic_performance" => "Afficher les performances académiques",

"select_promote_session" => "Sélectionner la promotion de l'année universitaire",

"select_promote_class" => "Sélectionner la classe de promotion",

"the_class_is_required" => "La classe est obligatoire",

"the_section_is_required" => "La section est obligatoire",

"select_promote_section" => "Sélectionner la section de promotion",

"promote" => "Promouvoir",

"all_student_export" => "Toutes les exportations d'étudiants",

"attendance_already_submitted_as_holiday" => "Participation déjà soumise en tant que jour férié. Vous pouvez éditer un enregistrement",

"attendance_already_submitted" => "Présence déjà soumise Vous pouvez éditer un enregistrement",

"unmark_holiday" => "Annuler la marque",

"add_student_group" => "Ajouter un groupe d'étudiants",

"edit_student_group" => "Editer le groupe d'étudiants",

"student_group_list" => "Liste des groupes d'étudiants",

"student_attendance_report" => "Rapport sur la participation des étudiants",

"january" => "Janvier",

"february" => "Février",

"march" => "Mars",

"april" => "Avril",

"may" => "Mai",

"june" => "Juin",

"july" => "Juillet",

"august" => "Août",

"september" => "Septembre",

"october" => "Octobre",

"november" => "Novembre",

"december" => "Décembre",

"student_category" => "Catégorie d'étudiant",

"promote_academic_year" => "Promouvoir l'année universitaire",

"select_current_class" => "Sélectionner la classe en cours",

"current_roll" => "Rouleau en cours",

"promote_class" => "Promotion de la classe",

"promote_section" => "Promouvoir la section",

"next_roll_number" => "Numéro de rouleau suivant",

"student_category_list" => "Liste des catégories d'étudiants",

"student_group" => "Groupe d'étudiants",

"disabled_student" => "Étudiants handicapés",

"birth_certificate" => "Certificat de naissance",

"student_edit" => "Éditer l'étudiant",

"kg" => "KG",

"add_parent" => "Ajouter un parent",

"siblings" => "Les frères",

"guardian_name" => "Nom des gardiens",

"guardian_email" => "Courrier électronique des gardiens",

"guardian_registration_message" => "Message d'enregistrement du Guardian",

"guardian_occupation" => "Profession des gardiens",

"guardian_address" => "Adresse du Guardian",

"student_address_info" => "Informations d'adresse de l'étudiant",

"vehicle_number" => "Numéro de véhicule",

"driver_name" => "Nom du pilote",

"update_student" => "Mettre à jour le",

"are_you" => "Etes-vous sûr de supprimer les éléments apparentés?",

"academic_records" => "Documents universitaires",

"merit" => "Mérite",

"academic" => "Académique",

"evalution" => "Evalution",

"marks_sheet_of" => "Feuille de notes de",

"ifsc_code" => "Code IFSC",

"import_student" => "Importer l'étudiant",

"save_student" => "Enregistrer l'étudiant",

"roll" => "Rouleau",

"manage_student" => "Gérer l'étudiant",

"disable_student" => "Désactiver l'étudiant",

"local_id_number" => "Numéro d'ID local",

"category_list" => "Liste des catégories",

"category_title" => "Titre de la catégorie",

"information_other" => "Informations autres",

"sibling_information" => "Informations de même niveau",

"parent_/_guardian_details" => "Détails du parent / du tuteur",

"attendance_result" => "Résultat de la participation",

"student_profile" => "Profil de l'élève",

"student_dashboard" => "Tableau de bord étudiant",

"download_sample_file" => "Télécharger un exemple de fichier",

"save_bulk_students" => "Enregistrer les étudiants en vrac",

"point1" => "Point1",

"point2" => "Point2",

"point3" => "Point3",

"point4" => "Point4",

"point5" => "Point5",

"point6" => "Point6",

"point7" => "Point7",

"point8" => "Point8",

"student_number" => "Numéro d'étudiant",

"transport" => "Transports",

"excel_file" => "Fichier Excel",

"student_import" => "Importation d'étudiant",

"all_student" => "Tous les étudiants",

"enable Student" => "Activer l'étudiant",

"enable_student" => "Activer l'étudiant",

"local_Id_Number" => "Numéro d'ID local",

"new_registration" => "Nouvel enregistrement",

"guardian_mobile" => "Gardien mobile",

"Title" => "Titre",

"Status" => "Statut",

"Actions" => "Actions",

"Class" => "Classe",

"Save" => "Sauvegarder",

"Time" => "Heure",

"Name" => "Nom",

"photo" => "Photo",

"name" => "Nom",

"settings" => "Paramètres",

"results" => "Résultats",

"required" => "Obligatoire",

"group" => "Groupe",

"remove" => "Supprimer",

"student" => "Étudiant",

"Current" => "Courant",

"Action" => "Action",

"Remove" => "Supprimer",

"Category" => "Catégorie",

"letter" => "Lettre",

"Info" => "Infos",

"Error" => "Erreur",

"Required" => "Obligatoire",

"status" => "Statut",

"value" => "Valeur",

"time" => "Heure",

"action" => "Action",

"class" => "Classe",

"section" => "Section",

"subject" => "Objet",

"room" => "Salle",

"save" => "Sauvegarder",

"start_time" => "Heure de début",

"contract_type" => "Type de contrat",

"bank_name" => "Nom de la banque",

"bank_account_number" => "Numéro de compte bancaire",

"paystack_note" => "Note de la pile",

"bank" => "Banque",

"select_criteria" => "Sélectionner les critères",

"are_you_sure_to_disable" => "Etes-vous sûr de désactiver cet élément?",

"are_you_sure_to_enable" => "Etes-vous sûr d'activer cet élément?",

"select_section" => "Sélectionner une section",

"admission" => "Admission",

"father" => "Père",

"date_of_birth" => "Date de naissance",

"student_id_card" => "Carte d'identité de l'étudiant",

"mother" => "Mère",

"blood_group" => "Groupe sanguin",

"relation_with_guardian" => "Relation avec Guardian",

"class_section" => "Classe (sec.)",

"admission_no" => "Droits d'entrée Non",

"first_name" => "Prénom",

"last_name" => "Nom de famille",

"crop" => "Culture",

"crop_image_and_upload" => "Image de culture et téléchargement",

"roll_number" => "Numéro de rouleau",

"chairman_of_the_examination_board" => "Président du comité d'examen",

"head_of_students_affairs" => "Chef des affaires étudiantes",

"is_present" => "Est présent",

"roll_no" => "Rouleau n °",

"pages" => "Pages",

"export_to_csv" => "Exporter au format CSV",

"export_to_excel" => "Exporter vers Excel",

"export_to_pdf" => "Exporter vers PDF",

"student_name" => "Nom de l'étudiant",

"title" => "Titre",

"height" => "Hauteur",

"width" => "Largeur",

"full" => "Plein",

"half" => "Demi",

"one_thired" => "Un seul souhaité",

"one_fourth" => "Un quatrième",

"Weight" => "Poids",

"applicable_user" => "Utilisateur applicable",

"page_layout" => "Mise en page",

"round" => "Cycle",

"squre" => "Squre",

"layout_spacing" => "Espacement de la présentation",

"top_space" => "Espace supérieur",

"bottom_space" => "Espace inférieur",

"left_space" => "Espace de gauche",

"right_space" => "Espace droit",

"horizontal" => "Horizontal",

"vertical" => "Verticale",

"guardian" => "Gardien",

"change_password" => "Modifier mot de passe",

"date_of_publication_of_result" => "Date de publication du résultat",

"error" => "Erreur",

"absent" => "Absent",

"actions" => "Actions",

"other" => "Autres",

"document" => "Document",

"student_list" => "Liste des étudiants",

"select_month" => "Sélectionner un mois",

"national_id_number" => "Carte d'identité nationale",

"delete_custom_field" => "Supprimer une zone personnalisée",

"birth_certificate_number" => "Numéro de certificat de naissance",

"email_address" => "Adresse électronique",

"academic_year" => "Année universitaire",

"select_class" => "Sélectionner une classe",

"select_subject" => "Sélectionner le sujet",

"select_academic_year" => "Sélectionner une année universitaire",

"info" => "Infos",

"add_timeline" => "Ajouter une chronologie",

"father_name" => "Nom du père",

"update_information" => "Sauvegarder les informations",

"search_by_name" => "Rechercher par nom",

"label" => "Libellé",

"parents_list" => "Liste des parents",

"teacher_list" => "Liste des enseignants",

"calendar" => "Calendrier",

"not_required" => "Non requis",

"phone_number" => "Numéro de téléphone",

"the_session_is_required" => "La session est requise",

"teachers" => "Enseignants",

"profile_update" => "Mettre à jour profil",

"personal_info" => "Informations personnelles",

"welcome_to" => "Bienvenue dans",

"delete_group" => "Supprimer le groupe",

"notice" => "Avis",

"students" => "Étudiants",

"parents" => "Parents",

"student_admission" => "Admission d'étudiants",

"student_attendance" => "Participation des étudiants",

"student_promote" => "Promotion de l'étudiant",

"holiday" => "Congés",

"student_registration" => "Inscription des étudiants",

"total_teachers" => "Nombre total d'enseignants",

"guardian_phone" => "Tuteurs Téléphone",

"room_name" => "Nom de la salle",

"exam" => "Exam",

"online_exam" => "Exam en ligne",

"passing_marks" => "Marques de passage",

"present" => "Présent",

"time_setup" => "Configuration du temps",

"Ending" => "Fin",

"ending" => "Fin",

"letter_grade" => "Grade Letter",

"highest_marks" => "Marques les plus élevées",

"obtained_marks" => "Marques observées",

"pass" => "Passe",

"fail" => "Echec",

"timeline" => "Chronologie",

"Parent_Guardian_Details" => "Détails du parent \/ du Guardian",

"full_marks" => "Marques complètes",

"visible_to_this_person" => "Visible pour cette personne",

"attendance" => "Participation",

"save_attendance" => "Enregistrer la participation",

"merit_list" => "Liste de mérite",

"save_group" => "Sauvegarder le groupe",

"update_group" => "Mettre à jour groupe",

"already_submitted" => "Déjà soumis",

"bank_payment" => "Paiement bancaire",

"category" => "Catégorie",

"invoice" => "Facture",

"course_category" => "Catégorie de cours",

"update_news_heading_section" => "Mettre à jour la section des rubriques",

"news_heading" => "Titre de la nouvelle",

"category_name" => "Nom de catégorie",

"update_category" => "Mettre à jour catégorie",

"delete_category" => "Supprimer une catégorie",

"course_heading" => "Titre du cours",

"course_details_heading" => "En-tête Détails du cours",

"incompleted" => "Interminée",

"summery" => "Summery",

"select_year" => "Sélectionner une année",

"current_address" => "Adresse actuelle",

"permanent_address" => "Adresse permanente",

"mode_of_payment" => "Mode de paiement",

"add_student" => "Ajouter un étudiant",

"mother_name" => "Nom de la mère",

"permanent" => "Permanent",

"document_info" => "Informations sur le document",

"documents" => "Documents",

"work_experience" => "Expérience de travail",

"resume" => "Reprendre",

"joining_letter" => "Lettre de joint",

"custom_field" => "Zone personnalisée",

"other_documents" => "Autre document",

"profile" => "Profil",

"document_title" => "Titre du document",

"upload_document" => "Télécharger le document",

"import_attendance" => "Importation de la participation",

"attendance_date" => "Date de participation",

"late" => "Late",

"add_note_here" => "Ajouter une note ici",

"mark_holiday" => "Marquer Holiday",

"half_day" => "La demi-journée",

"no_payroll_data" => "Aucune donnée de paie",

"not_leaves_data" => "Aucune donnée de congé",

"sell_receipt" => "Reçu de vente",

"purchase_receipt" => "Réception d'un ticket d'achat",

"bill_to" => "Bill To",

"sell_price" => "Prix de vente",

"sell_date" => "Date de vente",

"item_sale" => "Vente d'articles",

"save_category" => "Enregistrer la catégorie",

"issued_book" => "Livre émis",

"total_subject" => "Objet total",

"total_notice" => "Avis total",

"total_exam" => "Total Exam",

"total_online_exam" => "Total de l'examen en ligne",

"total_issued_book" => "Nombre total de livres émis",

"total_pending_home_work" => "Nombre total de travaux en attente",

"total_attendance_in_current_month" => "Nombre total de participations dans le mois en cours",

"pending_home_work" => "Travail à domicile en attente",

"attendance_in_current_month" => "Participation au mois en cours",

"student_report" => "Rapport étudiant",

"select_current_section" => "Sélectionner la section en cours",

"class_report_for_class" => "Classe de rapport pour la classe",

"Birth_Certificate_Number" => "Numéro de certificat de naissance",

"staring" => "Démarrage",

"without_additional" => "Sans supplémentaire",

"additional_subject" => "Objet supplémentaire",

"gpa_above" => "GPA au-dessus",

"in" => "Dans",

"information" => "Information",

];