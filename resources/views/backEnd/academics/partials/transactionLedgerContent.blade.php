...
<div class="invoice_wrapper">
    ...
    @endphp
    <div class="mt-30">
        <h4 class="mb-15">@lang('academics.order') &mdash; {{$invoice->invoice_id}} <small class="text-muted">({{ $invoice->create_date ? \Carbon\Carbon::parse($invoice->create_date)->format('M d, Y') : '' }}@php $invoiceSession = isset($sessions) ? $sessions->firstWhere('id', $invoice->academic_id) : null; @endphp{{ $invoiceSession ? ' — ' . $invoiceSession->year . ' [' . $invoiceSession->title . ']' : '' }})</small></h4>
        <table class="table border_table mb_30 description_table">
            <thead>
                <tr>
