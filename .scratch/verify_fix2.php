...
foreach ([5, 6] as $studentId) {
    ...
    $requiredThisTerm = $rows->filter(fn($r) => $r['classification'] !== 'minor');
    $eligible = $requiredThisTerm->filter(fn($r) => $r['unmet'] === 0);
    $hasSubmitted = $eligible->count() > 0 && $eligible->every(fn($r) => $r['chosen'] !== null);

    echo "Student #{$studentId} ({$student->first_name} {$student->last_name}): required(majors only)={$requiredThisTerm->count()} eligible={$eligible->count()} hasSubmitted=" . ($hasSubmitted ? 'TRUE' : 'FALSE') . "\n";
}
